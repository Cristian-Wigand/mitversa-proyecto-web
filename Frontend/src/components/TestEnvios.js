...
const TestEnvios = () => {
  ...
  useEffect(() => {
    axios
      .get('https://mitversa.christianferrer.me/api/envios/')
      .then((response) => {
    ...
    axios
      .get('https://mitversa.christianferrer.me/api/direcciones/')
      .then((response) => {
    ...
    axios
      .get('https://mitversa.christianferrer.me/api/comunas/')
      .then((response) => {
    ...
    axios
      .get('https://mitversa.christianferrer.me/api/ciudades/')
      .then((response) => {
  ...
  const verMapa = (envio) => {
    ...
    navigate(
      `/ruta/${envio.id_envio}?origen=${ciudadOrigen}&destino=${ciudadDestino}`,
    );
  };
    ...
    <div className="envios-container">
      {envios.map((envio, index) => (
        <div key={envio.id_envio} className="envio-card">
          <h3>Envío {index + 1}</h3>
          <p>
            <strong>Origen:</strong> Ciudad:{' '}
            {obtenerCiudadEnvio(envio.direccion_origen)}
          </p>
          <p>
            <strong>Destino:</strong> Ciudad:{' '}
            {obtenerCiudadEnvio(envio.direccion_destino)}
          </p>
          <p>
            <strong>Costo:</strong> ${envio.costo_total}
          </p>
          <p>
            <strong>Fecha de Inicio:</strong>{' '}
            {new Date(envio.fecha_pedido_inicio).toLocaleString()}
          </p>
          <p>
            <strong>Fecha de Fin:</strong>{' '}
            {new Date(envio.fecha_pedido_fin).toLocaleString()}
          </p>

...
const PackageCardCliente = () => {
  ...
  useEffect(() => {
    ...
    if (tipoUsuario !== 'cliente') {
      ...
      setSearchEnvio((prevState) => ({
        ...
        id_cliente: sessionStorage.getItem('userId'), // Asignamos el valor de 'userId'
      }));
    }
  }, [navigate]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      const id_usuario_c = sessionStorage.getItem('userId');
      console.log('id_usuario_c', id_usuario_c);

      try {
        const resultado = await conexiones.fetchSearch('Envio', {
          id_cliente: `${id_usuario_c}`,
        });
  ...
  const SubmitSearch = async (e) => {
    ...
    const resultado = await conexiones.fetchSearch('Envio', {
      id_cliente: searchEnvio['id_cliente'], // Mantener id_cliente constante
    ...
    // Aquí actualizamos el estado pero **sin modificar id_cliente**
    setSearchEnvio((prevState) => ({
      ...
      direccion_destino: '',
      costo_total: '',
      // id_cliente no se toca, ya que no lo estamos modificando
    }));
  };
      ...
      const data = await Promise.all(
        envioData.map(async (envio) => {
          const usuario = await Search('Usuario', {
            id_usuario: envio.id_repartidor,
          });
        ...
        {packageData.length === 0 ? (
          ...
          packageData.map((paquete) => {
            ...
            const usuario = usuarioData.find(
              (user) => user.id_usuario === result?.id_repartidor,
            );
...
export default PackageCardCliente;
